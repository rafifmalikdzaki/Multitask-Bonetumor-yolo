...
class MultiTaskLitModel(pl.LightningModule):
    def __init__(
        self, img_size: int = 640, lr: float = 1e-4, mask_log_period: int = 50
    ):
        super().__init__()
    ...
    def _multitask_loss(self, det_out, seg_out, targets, masks):
        ...
        if coeff_dim != proto_channels:
            print(
                f"Reshaping coefficients from {coeff_dim} to {proto_channels} channels"
            )
                ...
                if abs(height * width - coeff_dim) < 100:  # Allow some tolerance
                    try:
                        ...
                        pooled = F.adaptive_avg_pool2d(
                            flat_spatial, (proto_channels, 1)
                        )

                        # Reshape back to [B, Q, proto_channels]
                        coeffs_reshaped = pooled.reshape(
                            batch_size, num_queries, proto_channels
                        )

                        print(
                            f"Successfully reshaped using spatial dimensions {height}x{width}"
                        )
            ...
            if not found_spatial_arrangement:
                ...
                if (
                    abs(coeff_dim - (proto_height * proto_width * proto_channels))
                    < proto_channels
                ):
        ...
        if masks.shape[1] == 1 and seg_pred.shape[1] > 1:
            ...
            loss_seg = F.binary_cross_entropy_with_logits(
                seg_pred_resized, masks_expanded
            )
        else:
        ...
        if isinstance(det_out, (list, tuple)):
            ...
            for i, p in enumerate(det_out):
                ...
                tensor = _to_tensor(p)
                print(
                    f"Output tensor {i} shape: {tensor.shape if isinstance(tensor, torch.Tensor) else 'Not a tensor'}"
                )
                ...
                if isinstance(tensor, torch.Tensor):
                    ...
                    if len(tensor.shape) == 4:
                        ...
                        print(
                            f"Skipping non-detection tensor with shape: {tensor.shape}"
                        )
            ...
            if det_tensors:
                ...
                for det_idx, det_feat in enumerate(det_tensors):
                    ...
                    grid_y, grid_x = torch.meshgrid(
                        [
                            torch.arange(height, device=det_feat.device),
                    ...
                    x_center = (
                        torch.sigmoid(box_regression[..., 0]) + grid[..., 0]
                    ) * stride
                    y_center = (
                        torch.sigmoid(box_regression[..., 1]) + grid[..., 1]
                    ) * stride
                    w = (
                        torch.exp(box_regression[..., 2]) * stride
                    )  # Can use anchor multiplication if needed
                    h = (
                        torch.exp(box_regression[..., 3]) * stride
                    )  # Can use anchor multiplication if needed
                    ...
                    if det_feat.shape[-1] > 4:  # If we have objectness scores
                        ...
                        obj_scores = torch.ones(
                            batch_size, height, width, 1, device=det_feat.device
                        )
                        class_scores = obj_scores
            ...
            if isinstance(det_out, torch.Tensor):
                ...
                print(
                    f"Unexpected det_out type: {type(det_out)}, skipping detection loss"
                )
                return loss_seg
        ...
        if preds.shape[2] > 4:  # If we have more than just box coordinates
            ...
            print(
                f"Confidence stats: min={confidence_scores.min().item():.4f}, max={confidence_scores.max().item():.4f}, mean={confidence_scores.mean().item():.4f}"
            )
        ...
        for b in range(batch_size):
            ...
            if batch_preds.shape[1] > 4:
                ...
                if confident_mask.sum() > 0:
                    print(
                        f"Batch {b}: Keeping {confident_mask.sum().item()}/{batch_preds.shape[0]} predictions with confidence > {confidence_threshold}"
                    )
                    batch_preds = batch_preds[confident_mask]
                else:
                    print(
                        f"Batch {b}: No predictions above confidence threshold {confidence_threshold}, using all"
                    )
            ...
            print(
                f"Batch {b}: pred_boxes shape {pred_boxes.shape}, tgt_boxes shape {tgt_boxes.shape}"
            )

            # Vectorized IoU calculation for efficiency
            ious = torch.zeros(
                (len(pred_boxes), len(tgt_boxes)), device=pred_boxes.device
            )
            ...
            if len(pred_boxes) > 0 and len(tgt_boxes) > 0:
                ...
                print(
                    f"Batch {b}: IoU values stats - min: {ious.min().item():.4f}, max: {ious.max().item():.4f}, mean: {ious.mean().item():.4f}"
                )
        ...
        print(
            f"Segmentation loss: {loss_seg.item():.4f}, IoU loss: {loss_iou.item():.4f}, Total loss: {total_loss.item():.4f}"
        )
            ...
            if coeff_dim != proto_channels:
                ...
                if not found_spatial_arrangement:
                    if (
                        abs(
                            coeff_dim
                            - (protos.shape[2] * protos.shape[3] * proto_channels)
                        )
                        < proto_channels
                    ):
