...
class BTXRD(Dataset):
    def __init__(
        ...
        n = len(self.items)
        self.items = (
            self.items[: int(0.8 * n)]
            if split == "train"
            else self.items[int(0.8 * n) :]
        )
    ...
    def __getitem__(self, idx):
        ...
        mask_t = torch.from_numpy(mask[None]).float() / 127.0
...
def collate_fn(batch):
    ...
    img_cls = torch.stack(
        [
            torch.as_tensor(c)  # ensure Tensor
            for c in img_cls
        ]
    )
...
if __name__ == "__main__":
    ...
    train_ds = BTXRD(root_dir="btxrd_ready", split="train", img_size=640)
