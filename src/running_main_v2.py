...
def plot_confusion_matrix_to_wandb(cm_tensor, class_names_dict):
    if cm_tensor is None: return None
    class_names_list = [class_names_dict.get(i, str(i)) for i in range(cm_tensor.shape[0])]

    fig, ax = plt.subplots(figsize=(max(6, len(class_names_list)), max(5, len(class_names_list) * 0.8)))
    cm_np = cm_tensor.cpu().numpy()

    annotation_format = '.2f'

    sns.heatmap(cm_np, annot=True, fmt=annotation_format, cmap='Blues',
                xticklabels=class_names_list, yticklabels=class_names_list, ax=ax, annot_kws={"size": 8})
...
class MultiTaskLitModel(pl.LightningModule):
    def __init__(self,
        ...
        self.save_hyperparameters('img_size', 'lr', 'mask_log_period', 'box_log_period', 'cls_log_period',
                                  'nc_det', 'num_img_classes', 'proto_ch', 'loss_weight_seg',
                                  'loss_weight_box_iou', 'loss_weight_dfl', 'loss_weight_cls_det',
                                  'loss_weight_img_cls', 'iou_match_thresh', 'det_conf_thresh_viz',
                                  'map_max_detections')


        self.net = ConvNeXtBiFPNYOLO(nc_det=self.hparams.nc_det,
        ...
        self.train_img_acc = MulticlassAccuracy(num_classes=self.hparams.num_img_classes, average='micro',
                                                dist_sync_on_step=True)
        self.val_img_acc = MulticlassAccuracy(num_classes=self.hparams.num_img_classes, average='micro',
                                              dist_sync_on_step=True)
        self.val_img_cm = MulticlassConfusionMatrix(num_classes=self.hparams.num_img_classes, normalize='true')

        self.val_seg_f1 = TorchMetricsF1Score(task="binary", dist_sync_on_step=True)

        self.val_map_iou50_95 = MeanAveragePrecision(
            box_format='xyxy', class_metrics=True, iou_type="bbox",
            iou_thresholds=torch.linspace(0.5, 0.95, 10).tolist(),
            max_detection_thresholds=[1, 10, self.hparams.map_max_detections], # Adjusted based on typical use
            dist_sync_on_step=True,
        )
        self.val_map_iou50 = MeanAveragePrecision(
            box_format='xyxy', class_metrics=True, iou_type="bbox",
            iou_thresholds=[0.5],
            max_detection_thresholds=[1, 10, self.hparams.map_max_detections], # Adjusted
            dist_sync_on_step=True,
        )

        self.val_det_cm = MulticlassConfusionMatrix(num_classes=self.hparams.nc_det, normalize='true')
        self.temp_matched_preds_for_cm = []

        self.reg_max = self.net.detect.reg_max if hasattr(self.net.detect, 'reg_max') else 16

        self.img_class_names = {i: f"imgC{i}" for i in range(self.hparams.num_img_classes)}
    ...
    def _multitask_loss(self, det_head_outputs, seg_head_outputs, img_cls_logits_pred,
        ...
        elif len(seg_head_outputs) == 2 and isinstance(seg_head_outputs[1], (list, tuple)) and len(
                seg_head_outputs[1]) == 2:
            _det_output_from_seg_module, (_potentially_3d_mask_coeffs, actual_protos_tensor) = seg_head_outputs
        else:
            raise ValueError(
        ...
        loss_seg = self.seg_loss_fn(seg_logits_resized, gt_masks)  # Use local variable for loss
        ...
        for i, pred_fm in enumerate(det_head_outputs):
            ...
            grid_y, grid_x = torch.meshgrid(torch.arange(h, device=current_device, dtype=torch.float32),
                                            torch.arange(w, device=current_device, dtype=torch.float32), indexing='ij')
            ...
            stride_tensor_cat_list.append(
                torch.full((bs, h * w, 1), stride_val, device=current_device, dtype=torch.float32))
        ...
        if self.training: # Only clear for training, for validation it's cleared at epoch end
            self.temp_matched_preds_for_cm = []
        ...
        for b_idx in range(batch_size):
            ...
            if num_item_pos > 0:
                ...
                iou_for_loss = batch_bbox_iou(matched_pred_boxes, matched_gt_boxes).diag()
                ...
                cls_targets_one_hot = F.one_hot(matched_gt_classes, num_classes=self.hparams.nc_det).float()
                loss_cls_det_accum += self.det_cls_loss_fn(matched_pred_cls_logits, cls_targets_one_hot)
                ...
                if not self.training: # Check if we are in validation context via this call
                    self.temp_matched_preds_for_cm.extend(
                        list(zip(matched_pred_cls_logits.argmax(dim=-1).tolist(), matched_gt_classes.tolist()))
                    )
                ...
                ], dim=-1) / stride_tensor_item[positive_mask]
                ...
                for k_side in range(4):
                    loss_dfl_item_side_left = self.det_dfl_loss_fn(target_pred_dist_for_dfl[:, k_side, :],
                                                                   tl[:, k_side]) * wl[:, k_side]
                    loss_dfl_item_side_right = self.det_dfl_loss_fn(target_pred_dist_for_dfl[:, k_side, :],
                                                                    tr[:, k_side]) * wr[:, k_side]
                    loss_dfl_item += loss_dfl_item_side_left.sum() + loss_dfl_item_side_right.sum()
                loss_dfl_accum += loss_dfl_item

        avg_factor = num_total_pos_matches if num_total_pos_matches > 0 else float(batch_size) # Prevent division by zero
        ...
        total_loss = (self.hparams.loss_weight_seg * loss_seg +
    ...
    def training_step(self, batch, batch_idx):
        ...
        total_loss, loss_s, loss_b_iou, loss_dfl, loss_c_det, loss_icls = self._multitask_loss(
            det_head_outputs, seg_head_outputs, img_cls_logits_pred, det_boxes_gt, masks_gt, img_cls_gt)

        self.train_img_acc.update(img_cls_logits_pred, img_cls_gt)
        self.log_dict({
            "train/loss_total": total_loss, "train/loss_seg": loss_s, "train/loss_box_iou": loss_b_iou,
            "train/loss_dfl": loss_dfl, "train/loss_det_cls": loss_c_det, "train/loss_img_cls": loss_icls,
        ...
        if self.global_step > 0 and self.global_step % self.hparams.cls_log_period == 0:
            if self.logger and hasattr(self.logger.experiment, 'log'):
                log_cls_metrics(self.logger, img_cls_logits_pred, img_cls_gt,
        ...
        if self.global_step > 0 and self.global_step % self.hparams.mask_log_period == 0:
            if self.logger and hasattr(self.logger.experiment, 'log') and hasattr(self, 'seg_logits_for_logging'):
                log_seg_examples(self.logger, imgs, self.seg_logits_for_logging, # Use pre-computed logits
                                 masks_gt=masks_gt,
        ...
        if self.logger and hasattr(self.logger.experiment, 'log') and self.global_step > 0 and \
           (self.global_step % self.hparams.cls_log_period == 0 or \
            self.global_step % self.hparams.mask_log_period == 0):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # _multitask_loss will populate self.seg_logits_for_logging
        total_loss, loss_s, loss_b_iou, loss_dfl, loss_c_det, loss_icls = self._multitask_loss(
            det_outputs, seg_outputs, img_cls_logits, det_boxes_gt, masks_gt, img_cls_gt)
        ...
        elif len(seg_outputs) == 2 and isinstance(seg_outputs[1], (list, tuple)) and len(seg_outputs[1]) == 2:
            _, (_, actual_protos_tensor_val) = seg_outputs
        # ... (rest of seg_f1 logic) ...
        else:
            print(f"Warning: Unexpected seg_outputs structure in validation_step for seg_f1.")
            actual_protos_tensor_val = None  # Handle appropriately

        if actual_protos_tensor_val is not None and isinstance(actual_protos_tensor_val, torch.Tensor) and \
                actual_protos_tensor_val.ndim == 4 and actual_protos_tensor_val.shape[1] == self.hparams.proto_ch:
            seg_logits_projected_val = self.seg_proto_projector(actual_protos_tensor_val)
            seg_logits_resized_val = F.interpolate(seg_logits_projected_val,
                                                   size=(self.hparams.img_size, self.hparams.img_size),
                                                   mode='bilinear', align_corners=False)
            self.val_seg_f1.update(seg_logits_resized_val.sigmoid(), masks_gt.int())
        else:
            print(f"Warning: Skipping F1 update due to issues with actual_protos_tensor_val for seg_f1.")
        ...
        if not hasattr(self, 'project_val') or self.project_val.device != current_device_val:
            self.project_val = torch.arange(self.reg_max, device=current_device_val, dtype=torch.float32)

        # Decode detection outputs
        pred_boxes_batch_list, pred_cls_scores_batch_list, pred_cls_indices_batch_list = [], [], []
        ...
        for i_fm, pred_fm_val in enumerate(det_outputs):
            ...
            grid_y_v, grid_x_v = torch.meshgrid(torch.arange(h_v, device=current_device_val, dtype=torch.float32),
                                                torch.arange(w_v, device=current_device_val, dtype=torch.float32),
                                                indexing='ij')
            anchor_points_v = torch.stack((grid_x_v + 0.5, grid_y_v + 0.5), dim=-1).view(1, h_v * w_v, 2).repeat(bs_v,
                                                                                                                 1, 1)
            pred_boxes_xyxy_v = dist2bbox(decoded_ltrb_v * stride_val_v, anchor_points_v * stride_val_v)

            pred_boxes_batch_list.append(pred_boxes_xyxy_v)  # List of [B, N_level, 4]
            pred_cls_scores_batch_list.append(cls_logits_v.sigmoid())  # List of [B, N_level, Nc_det]
        ...
        for b_val_idx in range(batch_size_val):
            ...
            item_boxes_all_levels = torch.cat([level_preds[b_val_idx] for level_preds in pred_boxes_batch_list], dim=0)
            item_scores_all_levels_all_cls = torch.cat(
                [level_scores[b_val_idx] for level_scores in pred_cls_scores_batch_list], dim=0)

            # Get top score and corresponding class for each predicted box
            item_top_scores, item_top_labels = torch.max(item_scores_all_levels_all_cls, dim=1)

            # Clamp boxes
            item_boxes_clamped = item_boxes_all_levels.clamp(min=0, max=self.hparams.img_size)
            ...
            if gt_boxes_cxcywh_norm_val.numel() > 0:
                gt_boxes_xyxy_abs_val = torch.cat([
                    (gt_boxes_cxcywh_norm_val[:, 0] - gt_boxes_cxcywh_norm_val[:, 2] / 2) * self.hparams.img_size,
                    (gt_boxes_cxcywh_norm_val[:, 1] - gt_boxes_cxcywh_norm_val[:, 3] / 2) * self.hparams.img_size,
                    (gt_boxes_cxcywh_norm_val[:, 0] + gt_boxes_cxcywh_norm_val[:, 2] / 2) * self.hparams.img_size,
                    (gt_boxes_cxcywh_norm_val[:, 1] + gt_boxes_cxcywh_norm_val[:, 3] / 2) * self.hparams.img_size
                ], dim=-1).view(-1, 4)
                gt_boxes_xyxy_abs_val_clamped = gt_boxes_xyxy_abs_val.clamp(min=0, max=self.hparams.img_size)
            else:
                gt_boxes_xyxy_abs_val_clamped = torch.empty((0, 4), device=current_device_val, dtype=torch.float32)

            map_targets_for_metric.append({
                "boxes": gt_boxes_xyxy_abs_val_clamped,
            ...
            if gt_boxes_xyxy_abs_val_clamped.numel() > 0:
                gt_for_log = torch.cat([
                    ...
                    gt_classes_item_val.unsqueeze(1).float()  # Ensure float for cat
                ], dim=1)
        ...
        self.log_dict({
            "val/loss_total": total_loss, "val/loss_seg": loss_s, "val/loss_box_iou": loss_b_iou,
            "val/loss_dfl": loss_dfl, "val/loss_det_cls": loss_c_det, "val/loss_img_cls": loss_icls},
        ...
        if batch_idx == 0 and self.current_epoch % self.hparams.box_log_period == 0:
            if self.logger and hasattr(self.logger.experiment, 'log'):
                if det_preds_for_log:  # Check if there's something to log
                    log_det_examples(self.logger, imgs, det_preds_for_log, gts=det_gts_for_log,
                                     class_id_to_name=self.det_class_names,
                                     stage="val", step=self.global_step,
        ...
        if batch_idx == 0 and self.current_epoch % self.hparams.mask_log_period == 0:  # Or a different val log period
            if self.logger and hasattr(self.logger.experiment, 'log') and hasattr(self, 'seg_logits_for_logging'):
                log_seg_examples(self.logger, imgs, self.seg_logits_for_logging,
                                 masks_gt=masks_gt,
                                 stage="val", step=self.global_step, max_samples=MAX_VIZ_PER_CALL)

        # Manually commit if any loggers used commit=False
        if self.logger and hasattr(self.logger.experiment, 'log') and batch_idx == 0 and \
                (self.current_epoch % self.hparams.box_log_period == 0 or self.current_epoch % self.hparams.mask_log_period == 0):  # Check other conditions if val batch cls metrics are logged
            self.logger.experiment.log({}, commit=True, step=self.global_step)
    ...
    def on_validation_epoch_end(self):
        ...
        print(f"    Image accuracy logged: {val_img_accuracy:.4f} (took {time.time() - current_op_time:.4f}s)")
        ...
        print(f"    Image CM plotted & logged. (took {time.time() - current_op_time:.4f}s)")
        ...
        self.log("val_epoch/seg_f1_score_epoch", val_seg_f1_score, prog_bar=True, logger=True, sync_dist=True)
        print(f"    Segmentation F1 logged: {val_seg_f1_score:.4f} (took {time.time() - current_op_time:.4f}s)")
        ...
        try:
            ...
            print(f"    mAP@0.5:0.95 metrics computed. (took {time.time() - current_op_time:.4f}s)")
                ...
                if isinstance(v, torch.Tensor) and v.numel() == 1:
                    ...
                    for i_cls, ap_val in enumerate(v): # Renamed i to i_cls
                        if i_cls < self.hparams.nc_det:
                            log_map_dict_50_95[
                                f"val_epoch_map_iou50_95_class/{self.det_class_names.get(i_cls, f'cls_{i_cls}')}"] = ap_val.item()
            if log_map_dict_50_95:
                self.log_dict(log_map_dict_50_95, logger=True, sync_dist=True) # prog_bar=False for these detailed metrics
            print(f"    mAP@0.5:0.95 metrics logged. (took {time.time() - current_op_time_log:.4f}s for logging part)")
        else:
        ...
        try:
            ...
            print(f"    mAP@0.5 metrics computed. (took {time.time() - current_op_time:.4f}s)")
        except Exception as e_map_compute:
                ...
                if isinstance(v, torch.Tensor) and v.numel() == 1:
                    log_map_dict_50[f"val_epoch_map_iou50/{k}"] = v.item()
                     ...
                     for i_cls, ap_val in enumerate(v): # Renamed i to i_cls
                        if i_cls < self.hparams.nc_det:
                            log_map_dict_50[
                                f"val_epoch_map_iou50_class/{self.det_class_names.get(i_cls, f'cls_{i_cls}')}"] = ap_val.item()
            if log_map_dict_50:
                self.log_dict(log_map_dict_50, logger=True, sync_dist=True) # prog_bar=False
            print(f"    mAP@0.5 metrics logged. (took {time.time() - current_op_time_log:.4f}s for logging part)")
        else:
        ...
        if self.temp_matched_preds_for_cm:
            ...
            if preds_cm_list and gts_cm_list: # Check if lists are non-empty
                try:
                    ...
                    if det_cm_fig and self.logger and hasattr(self.logger.experiment, 'log'):
                        self.logger.experiment.log(
                            {"val_epoch/det_confusion_matrix_epoch": det_cm_fig, "global_step": self.global_step})
                ...
                print("    preds_cm_list or gts_cm_list became empty after zip, skipping det CM.")
        else:
            print("    No matched predictions stored for detection CM this epoch.")
        print(f"  Logged val_epoch/det_confusion_matrix_epoch (took {time.time() - current_op_time:.4f}s)")
        ...
        self.val_map_iou50_95.reset(); self.val_map_iou50.reset();
    ...
    def configure_optimizers(self):
        ...
        t_max_epochs = self.trainer.max_epochs if self.trainer and self.trainer.max_epochs is not None and self.trainer.max_epochs > 0 else 50
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=t_max_epochs,
...
class BTXRDDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_set, batch_size=self.hparams.batch_size, shuffle=True,
                          num_workers=self.hparams.num_workers, collate_fn=collate_fn,
...
if __name__ == "__main__":
    ...
    MAP_MAX_DETECTIONS = 100  # Max detections for mAP. Torchmetrics default is [1,10,100].
    ...
    wandb_logger = WandbLogger(project="BTXRD-MultiTask-AdvancedMetrics-v2", log_model="all") # New project name or version
    ...
    model = MultiTaskLitModel(
        ...
        nc_det=NC_DET, num_img_classes=NUM_IMG_CLASSES, proto_ch=PROTO_CH,
        loss_weight_seg=LOSS_WEIGHT_SEG, loss_weight_box_iou=LOSS_WEIGHT_BOX_IOU,
        loss_weight_dfl=LOSS_WEIGHT_DFL, loss_weight_cls_det=LOSS_WEIGHT_CLS_DET,
        loss_weight_img_cls=LOSS_WEIGHT_IMG_CLS, iou_match_thresh=IOU_MATCH_THRESH,
        det_conf_thresh_viz=DET_CONF_THRESH_VIZ,
        map_max_detections=MAP_MAX_DETECTIONS
    )

    trainer = pl.Trainer(
        max_epochs=MAX_EPOCHS, accelerator="auto", devices="auto", precision="16-mixed",
        ...
        logger=wandb_logger,
        callbacks=[lr_monitor, model_checkpoint, early_stopping]) # Add model_checkpoint_loss if using
