...
from ultralytics.utils.ops import non_max_suppression, process_mask_native
    ...
    def forward(self, x, mode: str = "train"):
        ...
        try:
            if mode == "train":
                ...
                nms_out = non_max_suppression(seg_preds_cat, 0.25, 0.7)            # ② prune
                return {                                                   # ③ expose
                    "boxes_xyxy":   [d[:, :4] for d in nms_out],
                    "scores":       [d[:, 4]  for d in nms_out],
                    "class_ids":    [d[:, 5].long() for d in nms_out],
                    "mask_coeffs":  [d[:, 6:] for d in nms_out],
                    "protos":       protos,
...
def _summ(t: torch.Tensor, k: int = 6) -> str:
    """Return dtype, shape and the first/last k flat values of a tensor."""
    flat = t.flatten()
    head = ", ".join([f"{v:.4g}" for v in flat[:k].tolist()])
    tail = ", ".join([f"{v:.4g}" for v in flat[-k:].tolist()]) if flat.numel() > k else ""
    dots = " … " if flat.numel() > 2 * k else ""
    return f"{str(t.dtype)[6:]} {tuple(t.shape)} = [{head}{dots}{tail}]"

def pp(name: str, obj):
    """Pretty-print any mixture of tensors, lists or dicts."""
    if isinstance(obj, torch.Tensor):
        print(f"{name}: {_summ(obj)}")
...
# ──────────── TRAIN forward pass ────────────
    print("\n──────── TRAIN mode ────────")
    net.train()
    seg_out, img_logits = net(dummy_img, mode="train")
    pp("seg_out", seg_out)
    pp("img_logits", img_logits)

# ──────────── INFER forward pass ────────────
    print("\n──────── INFER mode ────────")
    net.eval()
    with torch.no_grad():
        infer_dict = net(dummy_img, mode="infer")
    pp("infer_dict", infer_dict)
