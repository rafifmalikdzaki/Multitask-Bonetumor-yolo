...
class MultiTaskLitModel(pl.LightningModule):
    def __init__(self,
        ...
        self.val_map = MeanAveragePrecision(
            ...
            iou_type="bbox",
            dist_sync_on_step=True,
            max_detection_thresholds=[self.hparams.map_max_detections]  # Corrected argument
        )
    ...
    def _multitask_loss(self, det_head_outputs, seg_head_outputs, img_cls_logits_pred,
        ...
        avg_factor = num_total_pos_matches if num_total_pos_matches > 0 else float(batch_size)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step > 0 and self.global_step % self.hparams.mask_log_period == 0 and self.logger and hasattr(
                self.logger.experiment, 'log'):
            self._log_segmentation_example(imgs, seg_head_outputs, masks_gt, ids, batch_idx)
        return total_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        total_loss, loss_s, loss_b_iou, loss_dfl, loss_c_det, loss_icls = self._multitask_loss(
            det_train_out, seg_train_out, img_cls_train_logits, det_boxes_gt, masks_gt, img_cls_gt)
        ...
        if len(seg_train_out) == 3:
        ...
        elif len(seg_train_out) == 2 and isinstance(seg_train_out[1], (list, tuple)) and len(seg_train_out[1]) == 2:
            ...
            actual_protos_tensor_val = torch.zeros(imgs.shape[0], self.hparams.proto_ch,
                                                   self.hparams.img_size // 8, self.hparams.img_size // 8,
                                                   device=imgs.device)

        if not (isinstance(actual_protos_tensor_val, torch.Tensor) and actual_protos_tensor_val.ndim == 4):
            print(
        ...
        for b_val_idx in range(batch_size_val):
            item_boxes = torch.cat([level_preds[b_val_idx] for level_preds in pred_boxes_val_list], dim=0)
            item_scores_all_cls = torch.cat([level_scores[b_val_idx] for level_scores in pred_cls_scores_val_list],
                                            dim=0)
            item_scores, item_labels = torch.max(item_scores_all_cls, dim=1)

            map_preds_batch.append({"boxes": item_boxes.clamp(min=0, max=self.hparams.img_size),
                                    "scores": item_scores, "labels": item_labels})
            ...
            map_targets_batch.append({"boxes": gt_boxes_xyxy_abs_val.clamp(min=0, max=self.hparams.img_size),
        ...
        if batch_idx == 0 and self.current_epoch % self.hparams.box_log_period == 0:
            if map_preds_batch:
                self._log_detection_example(imgs, map_preds_batch, map_targets_batch, ids, batch_idx)
        return total_loss

    def on_train_epoch_end(self):
        # print(f"Epoch {self.current_epoch}: Training epoch end.") # Diagnostic
        self.log("train_epoch/img_accuracy", self.train_img_acc.compute(), prog_bar=True, logger=True, sync_dist=True)
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val_epoch/img_accuracy", val_img_accuracy, prog_bar=True, logger=True, sync_dist=True)
        ...
        if img_cm_fig and self.logger and hasattr(self.logger.experiment, 'log'):
            self.logger.experiment.log({"val_epoch/img_confusion_matrix": img_cm_fig, "global_step": self.global_step})
        print(f"    Image CM plotted & logged. (took {time.time() - current_op_time_plot:.4f}s)")

        current_op_time = time.time()
        print("  Computing and logging segmentation F1 score...")
        val_seg_f1_score = self.val_seg_f1.compute()
        self.log("val_epoch/seg_f1_score", val_seg_f1_score, prog_bar=True, logger=True, sync_dist=True)
        ...
        try:
            ...
            print(f"    mAP metrics computed. (took {time.time() - current_op_time:.4f}s)")
                ...
                if isinstance(v, torch.Tensor) and v.numel() == 1:
                    ...
                    for i, ap_val in enumerate(v):
                        if i < self.hparams.nc_det:
                            log_map_dict[
                                f"val_epoch_map_class/{self.det_class_names.get(i, f'cls_{i}')}"] = ap_val.item()

            if log_map_dict:
                self.log_dict(log_map_dict, logger=True, sync_dist=True)
            print(f"    mAP metrics logged. (took {time.time() - current_op_time_log:.4f}s for logging part)")
        else:
            ...
            if preds_cm_list and gts_cm_list:
                try:
                    ...
                    if det_cm_fig and self.logger and hasattr(self.logger.experiment, 'log'):
                        self.logger.experiment.log(
                            {"val_epoch/det_confusion_matrix": det_cm_fig, "global_step": self.global_step})
        ...
        print(f"  Logged val_epoch/det_confusion_matrix (took {time.time() - current_op_time:.4f}s)")
    ...
    def _log_segmentation_example(self, imgs, seg_head_outputs, gt_masks, ids, batch_idx):
        with torch.no_grad():
            if len(seg_head_outputs) == 3:
                ...
                actual_protos_tensor_viz = torch.zeros(imgs.shape[0], self.hparams.proto_ch,
                                                       self.hparams.img_size // 16, self.hparams.img_size // 16,
                                                       device=imgs.device)

            if not (isinstance(actual_protos_tensor_viz, torch.Tensor) and actual_protos_tensor_viz.ndim == 4 and
                    actual_protos_tensor_viz.shape[1] == self.hparams.proto_ch):
                print(
            ...
            seg_logits_projected_viz = self.seg_proto_projector(actual_protos_tensor_viz)
            seg_pred_resized_viz = F.interpolate(
                seg_logits_projected_viz, size=(self.hparams.img_size, self.hparams.img_size),
                mode='bilinear', align_corners=False)
            ...
            wandb_masks_data = {
                "predictions": {"mask_data": pred_mask_viz_prob, "class_labels": self.seg_class_labels_wandb},
                "ground_truth": {"mask_data": gt_mask_viz, "class_labels": self.seg_class_labels_wandb}
            }
            log_id = ids[0] if ids and len(ids) > 0 else f"train_batch{batch_idx}"
            if self.logger and hasattr(self.logger.experiment, 'log'):
                self.logger.experiment.log({
                    f"segmentation_examples_train/img_{log_id}": wandb.Image(img_to_log_chw, masks=wandb_masks_data),
                    "global_step": self.global_step
                })

    def _log_detection_example(self, imgs, map_preds_batch, map_targets_batch, ids, batch_idx):
        if not map_preds_batch or not map_targets_batch: return
        ...
        if "scores" in pred_item and pred_item["scores"].numel() > 0:
            ...
            for i in range(viz_boxes.shape[0]):
                ...
                pred_boxes_viz.append({
                    "position": {"minX": box[0] / self.hparams.img_size, "minY": box[1] / self.hparams.img_size,
                                 "maxX": box[2] / self.hparams.img_size, "maxY": box[3] / self.hparams.img_size},
        ...
        wandb_box_data = {}
        if pred_boxes_viz: wandb_box_data["predictions"] = {"box_data": pred_boxes_viz,
                                                            "class_labels": self.det_class_names}
        if gt_boxes_viz: wandb_box_data["ground_truth"] = {"box_data": gt_boxes_viz,
                                                           "class_labels": self.det_class_names}
        ...
        if wandb_box_data and self.logger and hasattr(self.logger.experiment, 'log'):
            self.logger.experiment.log({
                f"detection_examples_val/img_{log_id}": wandb.Image(img_to_log_chw, boxes=wandb_box_data),
                "global_step": self.global_step})
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=self.trainer.max_epochs if self.trainer and self.trainer.max_epochs > 0 else 50,
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(project="BTXRD-MultiTask-AdvancedMetrics", log_model="all")
    ...
    early_stopping = EarlyStopping(monitor='val/loss_total', patience=20, mode='min', verbose=True)
    ...
    model = MultiTaskLitModel(
        img_size=IMG_SIZE, lr=LEARNING_RATE, mask_log_period=MASK_LOG_PERIOD, box_log_period=BOX_LOG_PERIOD,
