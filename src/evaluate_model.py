...
from running_main_v2 import MultiTaskLitModel, BTXRDDataModule, plot_confusion_matrix_to_wandb, MAX_VIZ_PER_CALL
...
def evaluate(args):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() and args.accelerator == "cuda" else "cpu")
    ...
    try:
        ...
        model.map_detection_thresholds_list = [int(x) for x in args.map_thresholds.split(',')]
        print(f"Using custom mAP detection thresholds for this evaluation: {model.map_detection_thresholds_list}")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="Evaluating")):
            ...
            if actual_protos_tensor_test is not None and \
                    isinstance(actual_protos_tensor_test, torch.Tensor) and \
                    actual_protos_tensor_test.ndim == 4 and \
                    actual_protos_tensor_test.shape[1] == model.hparams.proto_ch:
                seg_logits_projected_test = model.seg_proto_projector(actual_protos_tensor_test)
                current_seg_logits = F.interpolate(seg_logits_projected_test,
                                                   size=(model.hparams.img_size, model.hparams.img_size),
                                                   mode='bilinear', align_corners=False)
                model.test_seg_f1.update(current_seg_logits.sigmoid().cpu(), masks_gt.int().cpu())

            # --- Detection Metrics & Logging ---
            map_preds, map_targets, det_log_preds, det_log_gts = \
                model._prepare_det_outputs_for_metrics_and_logging(det_outputs, det_boxes_gt, device, imgs.shape[0])
            ...
            _, _, _, _, _, _, matched_preds_for_cm_this_batch = model._multitask_loss(
                ...
                det_boxes_gt.to(device), masks_gt.to(device), img_cls_gt.to(device),
            ...
            if batch_idx == 0 and args.log_examples_wandb:
                ...
                if current_seg_logits is not None:
                    log_seg_examples(wandb_logger, imgs.cpu(), current_seg_logits.cpu(), masks_gt=masks_gt.cpu(),
                                     stage="test_eval", step=0, max_samples=MAX_VIZ_PER_CALL)
                if det_log_preds:
                    log_det_examples(wandb_logger, imgs.cpu(), det_log_preds, gts=det_log_gts,
                                     class_id_to_name=model.det_class_names, stage="test_eval", step=0,
                                     conf_th=model.hparams.det_conf_thresh_viz, max_samples=MAX_VIZ_PER_CALL)
                wandb_logger.experiment.log({}, commit=True, step=0)  # Commit example logs
    ...
    img_precision_macro = TMF.multiclass_precision(all_img_preds_cat, all_img_labels_cat, num_classes=num_img_classes,
                                                   average="macro").item()
    img_recall_macro = TMF.multiclass_recall(all_img_preds_cat, all_img_labels_cat, num_classes=num_img_classes,
                                             average="macro").item()
    img_f1_macro = TMF.multiclass_f1_score(all_img_preds_cat, all_img_labels_cat, num_classes=num_img_classes,
                                           average="macro").item()
    ...
    img_cm_fig = plot_confusion_matrix_to_wandb(img_cm_tensor_epoch.cpu(), model.img_class_names)
    if img_cm_fig:
        final_metrics["test_epoch/img_confusion_matrix"] = img_cm_fig
    print("Image Classification Confusion Matrix (normalized):\n", img_cm_tensor_epoch.cpu().numpy())
    ...
    for k, v in map_50_95_results.items():
        if isinstance(v, torch.Tensor) and v.numel() == 1:
            ...
            for i_cls, ap_val in enumerate(v):
                ...
                final_metrics[f"test_epoch_map_iou50_95_class/{cls_name}"] = ap_val.item()
    ...
    if model.test_temp_matched_preds_for_cm:
        preds_cm_list_test, gts_cm_list_test = zip(*model.test_temp_matched_preds_for_cm)
        if preds_cm_list_test and gts_cm_list_test:
            try:
                preds_tensor_test = torch.tensor(list(preds_cm_list_test), device="cpu", dtype=torch.long)
                gts_tensor_test = torch.tensor(list(gts_cm_list_test), device="cpu", dtype=torch.long)
                model.test_det_cm.update(preds_tensor_test, gts_tensor_test)  # metric should be on CPU for this
                det_cm_tensor_epoch = model.test_det_cm.compute()
                det_cm_fig = plot_confusion_matrix_to_wandb(det_cm_tensor_epoch.cpu(), model.det_class_names)
                if det_cm_fig:
                    final_metrics["test_epoch/det_confusion_matrix"] = det_cm_fig
                print("Detection Confusion Matrix (normalized):\n", det_cm_tensor_epoch.cpu().numpy())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate PyTorch Lightning MultiTask Model")
    parser.add_argument("--checkpoint_path", type=str, required=True, help="Path to the model checkpoint (.ckpt)")
    parser.add_argument("--root_dir", type=str, default="btxrd_ready", help="Root directory of the dataset")
    parser.add_argument("--img_size", type=int, default=640, help="Image size")
    parser.add_argument("--batch_size", type=int, default=4, help="Batch size for evaluation")
    parser.add_argument("--num_workers", type=int, default=2, help="Number of workers for DataLoader")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--accelerator", type=str, default="cuda", choices=["cuda", "cpu"],
                        help="Device to use (cuda or cpu)")
    parser.add_argument("--map_thresholds", type=str, default="1,10,50",
                        help="Comma-separated list of max detection thresholds for mAP (e.g., '1,10,50')")
    parser.add_argument("--log_examples_wandb", action="store_true", help="Log example images to W&B")
    parser.add_argument("--wandb_project", type=str, default="BTXRD-MultiTask-Evaluations", help="W&B project name")
    parser.add_argument("--wandb_run_name_suffix", type=str, default="", help="Suffix for W&B run name")
