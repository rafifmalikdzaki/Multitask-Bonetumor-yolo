...
class Bottleneck(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, shortcut: bool = True, groups: int=1, kernel: Tuple[int, int]=(3,3), e: float=0.5):
        super().__init__()
...
class DepthwiseConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, freeze_bn=False):
        super().__init__()
        self.depthwise = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups=in_channels, bias=False)
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, freeze_bn=False):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, autopad(kernel_size, padding, dilation), dilation, groups)
        self.bn = nn.BatchNorm2d(out_channels, momentum=0.9997, eps=4e-5)
        self.act = nn.SiLU()
...
class C2f(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, n: int=1, shortcut: bool=False, g: int=1, e: float=0.5):
        super().__init__()
        ...
        self.m = nn.ModuleList(
            Bottleneck(
                self.c, self.c, shortcut, groups=g, kernel=(3,3), e=1.0) for _ in range(n)
            )
...
class BiFPNUnit(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        p4_td_sum = w1_norm[0, 0] * p4_x + \
                    w1_norm[1, 0] * F.interpolate(p5_td_intermediate, scale_factor=2, mode='bilinear')
        p4_td = self.p4_td_cf(self.p4_td_conv(p4_td_sum))

        # Calculate P3_td: combines P3_x and upsampled P4_td
        p3_td_sum = w1_norm[0, 1] * p3_x + \
                    w1_norm[1, 1] * F.interpolate(p4_td, scale_factor=2, mode='bilinear')
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=None, dilation=1, groups=1, freeze_bn=False):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, autopad(kernel_size, padding, dilation), dilation, groups)
        self.bn = nn.BatchNorm2d(out_channels, momentum=0.9997, eps=4e-5)
        self.act = nn.SiLU()
...
class C2f(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, n: int=2, shortcut: bool=False, g: int=1, e: float=0.5):
        super().__init__()
        self.c = int(out_channels * e)
        self.cv1 = ConvBlock(in_channels, 2 * self.c, 1, 1)
        self.cv2 = ConvBlock((2 + n) * self.c, out_channels, 1)
...
class BiFPN(nn.Module):
    ...
    def __init__(self, size: List[int], feature_size: int = 256, num_layers: int = 3, eps: float = 0.0001):
        super().__init__()
        if not len(size) == 3:
            raise ValueError(f"BiFPN expects 3 input sizes for C3, C4, C5 projections, got {len(size)}")
    ...
    def forward(self, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        if not len(inputs) == 3:
            raise ValueError(f"BiFPN expects 3 input feature maps (from backbone C2f), got {len(inputs)}")
...
class ConvNeXtBiFPNYOLO(nn.Module):
    def __init__(self, nc_det: int, nc_img: int, proto_ch: int = 32,
                 bifpn_feature_size: int = 256, bifpn_num_layers: int = 2,
        ...
        self.neck = BiFPN(size=[256, 384, 512],
        ...
        self.detect = Detect(nc=nc_det, ch=head_input_channels)
        # For Segment head, npr is num_prototypes, nm is num_masks (mask_dim)
        self.segment = Segment(nc=nc_det, nm=proto_ch, npr=bifpn_feature_size, ch=head_input_channels)
    ...
    def forward(self, x, mode: str = "train"):
        ...
        original_detect_training_state = self.detect.training
        ...
        try:
            if mode == "train":
                ...
                detect_head_output = self.detect(head_inputs.copy())
                ...
                return detect_head_output, segment_head_output, img_cls_output
                ...
                # Detect head in eval mode typically returns (predictions_cat, feature_maps_list)
                det_preds_cat, det_feature_maps_list = self.detect(head_inputs.copy())
                ...
                return {
                    "detect_features": det_feature_maps_list,  # List of 3 feature maps from Detect head
                ...
                raise ValueError(f"Unknown mode for ConvNeXtBiFPNYOLO.forward: {mode}. Expected 'train' or 'infer'.")
        finally:
            self.detect.training = original_detect_training_state
...
# ───────────────────────────────────────────────────────────
@torch.no_grad()
def load_pretrained_heads(model: nn.Module,
                          detect_ckpt_path: str = None,
                          segment_ckpt_path: str = None):
    """
    ...
    def get_source_modulelist(ckpt_path):
        ...
        if hasattr(yolo_ckpt_model, 'model') and isinstance(yolo_ckpt_model.model, (nn.ModuleList, nn.Sequential)):
            src_nn_modulelist = yolo_ckpt_model.model
            print(f"  Successfully accessed model layers from yolo_ckpt_model.model (type: {type(src_nn_modulelist)}).")
        elif isinstance(yolo_ckpt_model, (nn.ModuleList, nn.Sequential)):
            src_nn_modulelist = yolo_ckpt_model
            print(f"  yolo_ckpt_model itself is an nn.ModuleList or nn.Sequential (type: {type(src_nn_modulelist)}).")
    ...
    def find_head_in_modulelist(module_list, head_class, head_name):
        ...
        for module in reversed(list(module_list)):
            if isinstance(module, head_class):
                ...
                return module
        print(f"  Could not find {head_name} of type {head_class.__name__} in the provided module list.")
        return None
    ...
    def copy_named_params(src_mod, dst_mod, mod_name_print=""):
        ...
        for name, dst_p in dst_mod.named_parameters():
            ...
            if name in src_dict:
                ...
                print(f"    Param '{name}' not found in source module {mod_name_print}.")
        return copied_count, total_params

    # --- Load Detect Head ---
    src_detect_head = None
    if detect_ckpt_path:
        detect_module_list = get_source_modulelist(detect_ckpt_path)
        if detect_module_list:
            src_detect_head = find_head_in_modulelist(detect_module_list, Detect, "Detect Head")

    det_c, det_t = 0, 0
    if hasattr(model, 'detect') and model.detect is not None:
        if src_detect_head is not None:
            det_c, det_t = copy_named_params(src_detect_head, model.detect, "Detect Head")
            print(f"Detect head          : {det_c}/{det_t} tensors copied from {detect_ckpt_path}")
        else:
            print(f"No source Detect head found or loaded from {detect_ckpt_path} to copy to model.detect.")
    elif not (hasattr(model, 'detect') and model.detect is not None):
        print("Destination model has no 'detect' attribute or it's None.")
    ...
    if segment_ckpt_path:
        ...
        if segment_module_list:
            src_segment_head = find_head_in_modulelist(segment_module_list, Segment, "Segment Head")
    ...
    if hasattr(model, 'segment') and model.segment is not None:
        if src_segment_head is not None:
            ...
            if hasattr(src_segment_head, 'cv4') and hasattr(model.segment, 'cv4'):
                if isinstance(src_segment_head.cv4, nn.ModuleList) and isinstance(model.segment.cv4, nn.ModuleList) and \
                        len(src_segment_head.cv4) == len(model.segment.cv4):
                    for i in range(len(src_segment_head.cv4)):
                        c, t = copy_named_params(src_segment_head.cv4[i], model.segment.cv4[i],
                                                 f"Segment.cv4[{i}] (Mask Coeffs Convs)")
                        seg_c_total += c;
                        seg_t_total += t
            elif hasattr(src_segment_head, 'mc') and hasattr(model.segment, 'mc'):
                c, t = copy_named_params(src_segment_head.mc, model.segment.mc, "Segment.mc (Mask Coeffs Convs)")
                seg_c_total += c;
                seg_t_total += t

            if hasattr(src_segment_head, 'proto') and hasattr(model.segment, 'proto'):
                c, t = copy_named_params(src_segment_head.proto, model.segment.proto,
            ...
            if hasattr(src_segment_head, 'cv2') and hasattr(model.segment, 'cv2'):
                if isinstance(src_segment_head.cv2, nn.ModuleList) and isinstance(model.segment.cv2, nn.ModuleList) and \
                        len(src_segment_head.cv2) == len(model.segment.cv2):
                    for i in range(len(src_segment_head.cv2)):
                        c, t = copy_named_params(src_segment_head.cv2[i], model.segment.cv2[i],
                                                 f"Segment.cv2[{i}] (Inherited Detect Bbox Reg)")
                        seg_c_total += c;
                        seg_t_total += t

            if hasattr(src_segment_head, 'cv3') and hasattr(model.segment, 'cv3'):
                if isinstance(src_segment_head.cv3, nn.ModuleList) and isinstance(model.segment.cv3, nn.ModuleList) and \
                        len(src_segment_head.cv3) == len(model.segment.cv3):
                    for i in range(len(src_segment_head.cv3)):
                        c, t = copy_named_params(src_segment_head.cv3[i], model.segment.cv3[i],
                                                 f"Segment.cv3[{i}] (Inherited Detect Class Pred)")
                        seg_c_total += c;
                        seg_t_total += t
            print(f"Segment head         : {seg_c_total}/{seg_t_total} tensors copied from {segment_ckpt_path}")
        else:
            print(f"No source Segment head found or loaded from {segment_ckpt_path} to copy to model.segment.")
    ...
    total_copied = det_c + seg_c_total
    total_attempted = det_t + seg_t_total
...
if __name__ == "__main__":
    ...
    net = ConvNeXtBiFPNYOLO(nc_det=NUM_DET_CLASSES, nc_img=NUM_IMG_CLASSES,
                            proto_ch=PROTO_CHANNELS, bifpn_feature_size=BIFPN_OUT_CH,
    ...
    if not os.path.exists(DETECT_CKPT_PATH):
        print(f"Warning: Detection checkpoint {DETECT_CKPT_PATH} not found. Detect head weights will not be loaded.")
    ...
    net = load_pretrained_heads(net,
                                detect_ckpt_path=DETECT_CKPT_PATH,
                                segment_ckpt_path=SEGMENT_CKPT_PATH)
    ...
    train_out_detect, train_out_segment_tuple, train_out_img_cls = net(dummy_img, mode="train")

    print(f"  Detect output type: {type(train_out_detect)}")
    if isinstance(train_out_detect, (list, tuple)):
        print(f"  Detect output length: {len(train_out_detect)}")
        for i, tnsr in enumerate(train_out_detect):
            print(f"    Item {i} shape: {tnsr.shape}")
    else:
        print(f"  Detect output shape: {train_out_detect.shape}")

    print(f"  Segment output (tuple) type: {type(train_out_segment_tuple)}")
    if isinstance(train_out_segment_tuple, (list, tuple)) and len(train_out_segment_tuple) == 3:
        ...
        if isinstance(seg_raw_det, (list, tuple)):
            for i, tnsr in enumerate(seg_raw_det): print(f"      Det internal item {i} shape: {tnsr.shape}")
    ...
    for key, value in infer_out_dict.items():
        if isinstance(value, torch.Tensor):
            ...
            print(f"  Key '{key}' type: {type(value)}, length: {len(value)}, First item shape: {value[0].shape}")
