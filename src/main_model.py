...
# ───────────────────────────────────────────────────────────
@torch.no_grad()
def load_pretrained_heads(
    model: nn.Module, detect_ckpt_path: str = None, segment_ckpt_path: str = None
):
    """
    ...
    if detect_ckpt_path:
        ...
        if detect_module_list:
            src_detect_head = find_head_in_modulelist(
                detect_module_list, Detect, "Detect Head"
            )

    det_c, det_t = 0, 0
    if hasattr(model, "detect") and model.detect is not None:
        if src_detect_head is not None:
            det_c, det_t = copy_named_params(
                src_detect_head, model.detect, "Detect Head"
            )
            print(
                f"Detect head          : {det_c}/{det_t} tensors copied from {detect_ckpt_path}"
            )
        else:
            print(
                f"No source Detect head found or loaded from {detect_ckpt_path} to copy to model.detect."
            )
...
if __name__ == "__main__":
    ...
    net = load_pretrained_heads(
        net, detect_ckpt_path=DETECT_CKPT_PATH, segment_ckpt_path=SEGMENT_CKPT_PATH
    )
    ...
    train_out_detect, train_out_segment_tuple, train_out_img_cls = net(
        dummy_img, mode="train"
    )
